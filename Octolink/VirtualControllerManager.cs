...
namespace Octolink
{
    ...
    public class VirtualControllerManager : IDisposable
    {
        ...
        private void UpdateXbox360Controller(int index, ControllerState state)
        {
            ...
            // Analog sticks (-32768 to 32767) - apply axis inversion
            float lx = InvertLeftX ? -state.LeftStickX : state.LeftStickX;
            float ly = InvertLeftY ? -state.LeftStickY : state.LeftStickY;
            float rx = InvertRightX ? -state.RightStickX : state.RightStickX;
            float ry = InvertRightY ? -state.RightStickY : state.RightStickY;
            
            controller.SetAxisValue(Xbox360Axis.LeftThumbX, (short)(lx * 32767));
            controller.SetAxisValue(Xbox360Axis.LeftThumbY, (short)(ly * 32767));
            controller.SetAxisValue(Xbox360Axis.RightThumbX, (short)(rx * 32767));
            controller.SetAxisValue(Xbox360Axis.RightThumbY, (short)(ry * 32767));
        ...
        private void UpdateDualShock4Controller(int ds4Index, ControllerState state)
        {
            ...
            // Analog sticks (0-255, 128 is center) - apply axis inversion
            float lx = InvertLeftX ? -state.LeftStickX : state.LeftStickX;
            float ly = InvertLeftY ? -state.LeftStickY : state.LeftStickY;
            float rx = InvertRightX ? -state.RightStickX : state.RightStickX;
            float ry = InvertRightY ? -state.RightStickY : state.RightStickY;
            
            controller.SetAxisValue(DualShock4Axis.LeftThumbX, (byte)((lx + 1) * 127.5f));
            controller.SetAxisValue(DualShock4Axis.LeftThumbY, (byte)((1 - ly) * 127.5f)); // Y is inverted
            controller.SetAxisValue(DualShock4Axis.RightThumbX, (byte)((rx + 1) * 127.5f));
            controller.SetAxisValue(DualShock4Axis.RightThumbY, (byte)((1 - ry) * 127.5f)); // Y is inverted
